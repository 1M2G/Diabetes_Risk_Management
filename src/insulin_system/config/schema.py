...
class FeatureEngineeringConfig:
    ...
    SELECTION_K: Optional[int] = 30  # Keep top K features; None = no limit for statistical selection
...
class ModelConfig:
    ...
    minority_class_weight_multiplier: float = 2.0
...
GLUCOSE_ZONES: List[Dict[str, Any]] = [
    {"id": "hypo", "min_mg_dl": None, "max_mg_dl": GLUCOSE_HYPO_MAX_MGDL, "label": "Below 70", "interpretation": "Hypoglycemia (Low)", "action": f"Stop. Suspend all insulin logic. Prompt user to consume {FAST_ACTING_CARBS_GRAMS}g fast-acting carbs.", "severity": "critical"},
...
class DashboardConfig:
    ...
    data_path: Path = field(default_factory=lambda: Path("insulin_dosage_prediction.csv"))
    similar_patients_k: int = 5

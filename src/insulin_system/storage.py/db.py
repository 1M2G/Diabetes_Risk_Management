...
def insert_record(
    ...
    try:
        cur = conn.execute(
            """INSERT INTO records (created_at, endpoint, request_id, predicted_class, confidence, is_high_risk, input_summary, response_summary)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
            (
...
def get_records(limit: int = 100, db_path: Optional[Path] = None) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        cur = conn.execute(
            "SELECT id, created_at, endpoint, request_id, predicted_class, confidence, is_high_risk, input_summary, response_summary FROM records ORDER BY id DESC LIMIT ?",
            (limit,),
        )
        rows = cur.fetchall()
        out = []
...
def get_glucose_readings(hours: int = 72, db_path: Optional[Path] = None) -> List[Dict[str, Any]]:
    path = get_db_path(db_path)
    if not path.exists():
        return []
    import sqlite3
    ...
    try:
        conn.row_factory = sqlite3.Row
        cur = conn.execute(
            "SELECT id, reading_at, value, is_predicted FROM glucose_readings WHERE reading_at >= ? ORDER BY reading_at ASC",
            (since,),
        )
        return [dict(r) for r in cur.fetchall()]
    finally:
...
def insert_glucose_reading(
    ...
    try:
        cur = conn.execute(
            "INSERT INTO glucose_readings (reading_at, value, is_predicted) VALUES (?, ?, ?)",
            (datetime.now(timezone.utc).isoformat(), int(round(float(value))), 1 if is_predicted else 0),
        )
        conn.commit()
        return cur.lastrowid
    finally:
...
def insert_dose_event(
    ...
    try:
        cur = conn.execute(
            """INSERT INTO dose_events (created_at, meal_bolus, correction_dose, total_dose, request_id)
               VALUES (?, ?, ?, ?, ?)""",
            (datetime.now(timezone.utc).isoformat(), meal_bolus, correction_dose, total_dose, request_id),
        )
        conn.commit()
        return cur.lastrowid
    finally:

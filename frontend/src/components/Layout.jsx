...
import { FiBell, FiAlertTriangle, FiUser, FiChevronDown, FiActivity, FiGrid, FiTrendingUp, FiFileText, FiSliders, FiSun, FiMenu } from 'react-icons/fi'
...
function TopBar({ sidebarOpen, onToggleSidebar, onLogoTripleClick }) {
  ...
  useEffect(() => {
    if (showSettings) fetch(`${API}/settings`).then((r) => r.ok && r.json()).then((d) => d && setSettings(d)).catch(() => {})
  }, [showSettings])
  ...
  const saveSettings = (key, value) => {
    ...
    fetch(`${API}/settings`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(next) }).catch(() => {})
  }

  const goToAlerts = () => { setShowNotifications(false); navigate('/alerts') }
  const goToReports = () => { setShowNotifications(false); navigate('/reports') }
  ...
  const handleSignOut = () => { setShowProfile(false); setSignedIn(false) }
    ...
    <header className="topbar">
      <div className="topbar-brand">
        ...
        <div>
          ...
          <span className="topbar-subtitle">Clinical Decision Support</span>
        </div>
      </div>
...
function Sidebar({ isOpen, onClose }) {
  const { patient, recentMetrics } = useClinical()
  const navigate = useNavigate()

  const GLUCOSE_LOW_ABNORMAL = 70
  const GLUCOSE_HIGH_ABNORMAL = 180
  const isAbnormalGlucose = (v) => v != null && (v < GLUCOSE_LOW_ABNORMAL || v > GLUCOSE_HIGH_ABNORMAL)
  const isAbnormalActivity = (v) => v != null && v > 120

  const gl = recentMetrics.glucose
  const carbs = recentMetrics.carbohydrates ?? recentMetrics.carbohydrate
  const activity = recentMetrics.activityMinutes ?? recentMetrics.activity
  ...
  return (
    <aside className={`sidebar sidebar-flip ${isOpen ? 'sidebar-flip--open' : ''}`} aria-hidden={!isOpen}>
      ...
      <div className="sidebar-metrics">
        <h4 className="sidebar-metrics-title">Recent metrics</h4>
        <div className={`sidebar-metric-row ${isAbnormalGlucose(gl) ? 'abnormal' : ''}`}>
          <span className="sidebar-metric-label">Glucose</span>
          <span className="sidebar-metric-value">{gl != null ? `${gl} ${recentMetrics.glucoseUnit || 'mg/dL'}` : '—'}</span>
        </div>
        <div className="sidebar-metric-row">
          <span className="sidebar-metric-label">Carbohydrates</span>
          <span className="sidebar-metric-value">{carbs != null ? `${carbs} g` : '—'}</span>
        </div>
        <div className={`sidebar-metric-row ${isAbnormalActivity(activity) ? 'abnormal' : ''}`}>
          <span className="sidebar-metric-label">Activity</span>
          <span className="sidebar-metric-value">{activity != null ? `${activity} min` : '—'}</span>
        </div>
        {recentMetrics.timestamp && (
          <div className="sidebar-metrics-time">
            {new Date(recentMetrics.timestamp).toLocaleString(undefined, { dateStyle: 'short', timeStyle: 'short' })}
          </div>
        )}
      </div>

      <nav className="sidebar-nav" aria-label="Main" onClick={handleNavClick}>
        <NavLink to="/" end className="sidebar-nav-link"><FiGrid size={18} className="sidebar-nav-icon" /> Dashboard</NavLink>
        <NavLink to="/glucose-trends" className="sidebar-nav-link"><FiTrendingUp size={18} className="sidebar-nav-icon" /> Glucose Trends</NavLink>
        <NavLink to="/insulin-management" className="sidebar-nav-link"><FiActivity size={18} className="sidebar-nav-icon" /> Glucose & Dosage</NavLink>
        <NavLink to="/reports" className="sidebar-nav-link"><FiFileText size={18} className="sidebar-nav-icon" /> Reports</NavLink>
        <NavLink to="/alerts" className="sidebar-nav-link"><FiAlertTriangle size={18} className="sidebar-nav-icon" /> Alerts</NavLink>
      </nav>
    </aside>
  )
}
